import type { LogLevel, Logger } from '@cashu/cashu-ts';
...
function getLogLevel(name: string, suffix?: string): LogLevelWithOff {
	const envKey = suffix ? `${name.toUpperCase()}_${suffix.toUpperCase()}_LOG_LEVEL` : `${name.toUpperCase()}_LOG_LEVEL`;
	...
	return parseLogLevel(
		...
		parseLogLevel(
			serviceLevel,
			parseLogLevel(globalLevel, 'info')
		)
	);
}
